...
const malaysiaAirports = [
    { code: 'KUL', name: 'Kuala Lumpur (KLIA)' }, { code: 'SZB', name: 'Subang (Skypark)' },
    { code: 'SDK', name: 'Sandakan' }, { code: 'BKI', name: 'Kota Kinabalu' },
    { code: 'TWU', name: 'Tawau' }, { code: 'LDU', name: 'Lahad Datu' },
    { code: 'KCH', name: 'Kuching' }, { code: 'MYY', name: 'Miri' },
    { code: 'BTU', name: 'Bintulu' }, { code: 'SBW', name: 'Sibu' },
    { code: 'PEN', name: 'Pulau Pinang' }, { code: 'JHB', name: 'Johor Bahru (Senai)' },
    { code: 'LGK', name: 'Langkawi' }, { code: 'KBR', name: 'Kota Bharu' },
    { code: 'TGG', name: 'Kuala Terengganu' }, { code: 'KUA', name: 'Kuantan' },
    { code: 'AOR', name: 'Alor Setar' }, { code: 'LBU', name: 'Labuan' },
    { code: 'IPH', name: 'Ipoh' }, { code: 'SIN', name: 'Singapura (Changi)' },
    { code: 'BPN', name: 'Balikpapan' }, { code: 'JED', name: 'Jeddah' }
];
...
function App() {
    ...
    useEffect(() => {
        if (isGantiDateLocked) {
            setFormData(prev => ({
                ...prev, tarikhGantiDari: prev.tarikhPergi, tarikhGantiHingga: prev.tarikhBalik,
                flightPergiTarikh: prev.tarikhPergi, flightBalikTarikh: prev.tarikhBalik
            }));
        } else {
            setFormData(prev => ({
                ...prev, flightPergiTarikh: prev.tarikhPergi, flightBalikTarikh: prev.tarikhBalik
            }));
        }
        ...
        for (let field of requiredFields) {
            if (!field.val || (typeof field.val === 'string' && field.val.trim() === '')) {
                ...
                if (!expanded[sectionName]) {
                    setExpanded(prev => ({
                        pegawai: false, tugas: false, pengganti: false, tiket: false, cuti: false, peranan: false, tandatangan: false,
                        [sectionName]: true
                    }));
                }
    ...
    const toggleAutoFieldsEdit = () => {
        if (isEditingAutoFields) {
            ...
            if (selected) {
                setFormData(prev => ({
                    ...prev, jawatan: selected.jawatan, bahagian: selected.bahagian, noTel: selected.noTel || prev.noTel
                }));
            }
        }
    ...
    // PEMBETULAN: Paksa jenis String untuk mengelak jsPDF error
    const val = (text) => (text && text.toString().trim() !== '') ? text.toString() : '-';
    ...
    const generateForm1 = (doc, logoImgBase64) => {
        ...
        doc.setFontSize(10);
        doc.text("JABATAN TENAGA MANUSIA", 105, currentY, { align: 'center' }); currentY += 6;
        doc.text("BORANG KEBENARAN MENJALANKAN TUGASAN RASMI DILUAR IBU PEJABAT", 105, currentY, { align: 'center' }); currentY += 4.5;
        doc.setFont("helvetica", "normal"); doc.setFontSize(8.5);
        doc.text("( Borang ini hendaklah diisi sebelum memulakan perjalanan )", 105, currentY, { align: 'center' }); currentY += 10;
        ...
        doc.text("Jawatan", 28, currentY); doc.text(":", 75, currentY); doc.text(val(formData.jawatan), 78, currentY); currentY += 6;
        doc.text("3.", 18, currentY); doc.text("Bahagian/Unit", 28, currentY);
        doc.text(":", 75, currentY); doc.text(val(formData.bahagian), 78, currentY); currentY += 6;
        doc.text("4.", 18, currentY); doc.text("No. Pendaftaran/Jenis Kenderaan", 28, currentY); doc.text(":", 75, currentY);
        doc.text(val(formData.noKenderaan), 78, currentY); currentY += 6;
    ...
    const generateForm2 = (doc, logoImgBase64) => {
        doc.setFont("helvetica");
        doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.text("LAMPIRAN 7", 190, 15, { align: 'right' });
        ...
        doc.text("JABATAN TENAGA MANUSIA", 105, currentY, { align: 'center' }); currentY += 5;
        const text1 = "ADTEC : ";
        const text2 = "ADTEC JTM KAMPUS SANDAKAN"; const totalWidth = doc.getTextWidth(text1 + text2);
        const startX = 105 - (totalWidth / 2);
        doc.text(text1 + text2, 105, currentY, { align: 'center' });
        doc.line(startX + doc.getTextWidth(text1), currentY + 1, startX + totalWidth, currentY + 1); currentY += 8;
    ...
    const generateForm3 = (doc) => {
        doc.setFont("helvetica");
        doc.setDrawColor(0); doc.setLineWidth(0.3); doc.rect(130, 15, 65, 12);
        ...
        if (formData.tandatangan) {
            try { doc.addImage(formData.tandatangan, 'PNG', 140, 115, 40, 20); } catch(e) {}
        }
        ...
        doc.setFontSize(9);
        doc.text("KELAYAKAN TAMBANG:", 15, currentY); doc.text("** B/H/Y", 60, currentY); currentY += 8;
        doc.text("HARGA TAMBANG", 15, currentY); doc.text(":", 50, currentY);
        doc.rect(55, currentY - 4, 30, 6); doc.text("RM", 57, currentY);
        doc.text("KOD JENIS", 100, currentY); doc.text(":", 135, currentY);
        doc.rect(140, currentY - 4, 45, 6); doc.text(upperVal(formData.kodSyarikat), 142, currentY); currentY += 8;
        doc.text("CUKAI AIRPORT", 15, currentY); doc.text(":", 50, currentY);
        doc.rect(55, currentY - 4, 30, 6); doc.text("RM", 57, currentY);
        doc.text("MAS / AIR ASIA", 100, currentY); doc.text(":", 135, currentY);
        doc.rect(140, currentY - 4, 45, 6); currentY += 8;
    ...
    const generateFormCuti = (doc) => {
        ...
        currentY += 8;
        doc.setFont("helvetica", "bold");
        doc.text("Kepada:", 15, currentY); doc.setFont("helvetica", "normal");
                ...
                <div className="max-w-5xl mx-auto w-full relative z-10 pt-10 pb-16">
                    <div className="text-center mb-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
                            <button 
                                onClick={() => { setActiveForm('tugas'); setExpanded({...expanded, cuti: false, peranan: false, tandatangan: false, tugas: false, pegawai: true}); }}
                            ...
                            <button 
                                onClick={() => { setActiveForm('akujanji'); setExpanded({...expanded, tugas: false, tiket: false, cuti: false, pengganti: false, pegawai: true}); }}
                            ...
                            <button 
                                onClick={() => { setActiveForm('cuti'); setExpanded({...expanded, tugas: false, peranan: false, tandatangan: false, pegawai: true}); }}
    ...
    return (
        <div className="pb-12 relative min-h-screen">
            ...
            <header className="relative pt-16 pb-10 px-6 max-w-3xl mx-auto text-center">
                <div className={`inline-flex items-center justify-center p-3 rounded-2xl mb-6 text-white shadow-xl transform -rotate-3 hover:rotate-0 transition-transform duration-300 ${activeForm === 'cuti' ? 'bg-emerald-600 shadow-emerald-500/30' : activeForm === 'akujanji' ? 'bg-indigo-600 shadow-indigo-500/30' : 'bg-blue-600 shadow-blue-500/30'}`}>
                    {activeForm === 'cuti' ? (
                ...
                <h1 className="text-2xl md:text-4xl font-extrabold text-slate-900 tracking-tight leading-tight">
                    {activeForm === 'cuti' ? 'Borang Cuti Manual' : activeForm === 'akujanji' ? 'Surat Akujanji Peperiksaan' : 'Borang Tugas Rasmi'} <br />
                    <span className={`text-transparent bg-clip-text bg-gradient-to-r ${activeForm === 'cuti' ? 'from-emerald-600 to-teal-500' : activeForm === 'akujanji' ? 'from-indigo-600 to-purple-600' : 'from-blue-600 to-indigo-600'}`}>ADTEC JTM Kampus Sandakan</span>
                </h1>
            ...
            <div className="max-w-[800px] mx-auto px-6 mb-10 relative z-20">
                <div className="relative">
                    ...
                    <div className={`absolute top-5 left-0 h-1.5 rounded-full bg-gradient-to-r transition-all duration-1000 ease-out -translate-y-1/2 ${activeForm === 'cuti' ? 'from-blue-500 to-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]' : activeForm === 'akujanji' ? 'from-blue-500 via-indigo-500 to-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.4)]' : 'from-blue-500 via-indigo-500 via-teal-500 to-emerald-500 shadow-[0_0_10px_rgba(99,102,241,0.4)]'}`} style={{ width: `${progressWidth}%` }}></div>
            ...
            <div className="max-w-[800px] mx-auto px-4 space-y-5 relative z-10">
                        ...
                        <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7">
                                <div className="md:col-span-2">
                                    ...
                                    <div className="relative">
                                        <select 
                                            id="wrap-nama" name="nama" value={formData.nama} onChange={handleChange}
                                ...
                                <div>
                                    <label className={formLabelClass}>
                                        ...
                                        {isKnownStaff && (
                                            <button 
                                                type="button" onClick={toggleAutoFieldsEdit}
                                    ...
                                    <input 
                                        id="wrap-jawatan" type="text" name="jawatan" value={formData.jawatan} onChange={handleChange} 
                                        ...
                                        placeholder="Contoh: Pengajar" readOnly={isKnownStaff && !isEditingAutoFields} 
                                    />
                                </div>
                                <div>
                                    <UniversalSelect 
                                        wrapperId="wrap-bahagian" name="bahagian" value={formData.bahagian} 
                                        ...
                                        options={unitOptions} onChange={handleChange} placeholder="Pilih Unit" disabled={isKnownStaff && !isEditingAutoFields} 
                                    />
                                </div>
                                <div>
                            ...
                            <div className="mt-8 flex justify-end">
                                <button onClick={() => nextSection('pegawai', activeForm === 'cuti' ? 'cuti' : activeForm === 'akujanji' ? 'peranan' : 'tugas')} className="bg-slate-900 hover:bg-slate-800 text-white font-bold py-3 px-6 rounded-xl shadow-md transition-all active:scale-95 flex items-center gap-2">
                        ...
                        {expanded.cuti && isPegawaiComplete && (
                            <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7">
                                    <div className="md:col-span-2">
                                        <UniversalSelect 
                                            wrapperId="wrap-jenisCuti" name="jenisCuti" value={formData.jenisCuti} 
                                            label={<>Jenis Cuti <span className="text-red-500">*</span></>} 
                                            options={["Cuti Rehat", "Cuti Kecemasan", "Cuti Tanpa Rekod"]} onChange={handleChange} 
                                        />
                                    </div>
                                    <div>
                                    ...
                                    <div className="md:col-span-2 mt-4 pt-6 border-t border-slate-100">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <select id="wrap-ketuaSokongan" name="ketuaSokongan" value={formData.ketuaSokongan} onChange={handleChange} className={`block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-[15px] font-semibold shadow-sm transition-all duration-300 focus:border-blue-500 focus:outline-none focus:ring-[4px] focus:ring-blue-500/10 appearance-none relative z-10 cursor-pointer ${formData.ketuaSokongan ? 'text-slate-800' : 'text-slate-400 font-medium'}`}>
                                            ...
                                            <div>
                                                ...
                                                <div className="relative">
                                                    <select id="wrap-pegawaiPelulus" name="pegawaiPelulus" value={formData.pegawaiPelulus} onChange={handleChange} className={`block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-[15px] font-semibold shadow-sm transition-all duration-300 focus:border-blue-500 focus:outline-none focus:ring-[4px] focus:ring-blue-500/10 appearance-none relative z-10 cursor-pointer ${formData.pegawaiPelulus ? 'text-slate-800' : 'text-slate-400 font-medium'}`}>
                                                        <option value="" disabled>-- Sila Pilih --</option>
                                                        {[...pegawaiDatabase].filter(p => p.nama !== formData.nama).sort((a,b) => a.nama.localeCompare(b.nama)).map((p, idx) => (
                                        ...
                                        <div className="flex flex-col gap-3">
                                            {peperiksaanRoles.map((role, idx) => (
                                                <label key={idx} className={`flex items-center gap-4 p-3 rounded-xl border-2 cursor-pointer transition-all ${formData.perananPeperiksaan.includes(role) ? 'bg-indigo-50 border-indigo-400' : 'bg-white border-slate-200 hover:border-indigo-300'}`}>
                                                    <input type="checkbox" checked={formData.perananPeperiksaan.includes(role)} onChange={() => handleCheckboxPeranan(role)} className="custom-checkbox flex-shrink-0" />
                                ...
                                <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
                                    <div id="wrap-tandatangan" className="bg-slate-50 border border-slate-200 rounded-[1.5rem] p-4 sm:p-6 transition-all duration-300">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                                            ...
                                            <div className="relative">
                                                <input type="file" accept="image/*" onChange={handleSignatureUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                                        ...
                                        <div className="relative">
                                            <canvas 
                                                ref={canvasRef} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={stopDrawing}
                                        ...
                                        <div className="flex justify-between items-center mt-3">
                                            ...
                                            <button onClick={clearSignature} className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-[12px] font-bold rounded-lg transition-colors flex items-center gap-1">
                                ...
                                <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
                                    <div className="mb-8">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
                                            {quickTemplates.map((t, idx) => (
                                                <button key={idx} onClick={() => setFormData(prev => ({ ...prev, tujuan: t.tujuan, caraPerjalanan: t.perjalanan }))} className="flex flex-col items-center justify-center p-3 rounded-2xl bg-white border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all text-slate-600 font-bold text-[13px] gap-2 active:scale-95 shadow-sm">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7">
                                        ...
                                        <div className="md:col-span-2 pt-4 border-t border-slate-100">
                                            <UniversalSelect name="caraPerjalanan" value={formData.caraPerjalanan} label="Cara Perjalanan Pilihan" options={["Kereta Sendiri", "Kereta Jabatan", "Kereta Rasmi Jawatan", "Kapal Terbang", "Lain-lain"]} onChange={handleChange} placeholder="Sila Pilih" />
                                        </div>
                                        <div className={`md:col-span-2 transition-all duration-500 overflow-hidden ${formData.caraPerjalanan === 'Kereta Sendiri' ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                            <div className="bg-slate-50 border border-slate-200 rounded-[1.5rem] p-6 mt-2">
                                                ...
                                                <div className="mt-6">
                                                    <label className="block text-[13px] font-extrabold text-slate-800 mb-3">Jika perjalanan melebihi 240km, sila tandakan sebab:</label>
                                ...
                                <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
                                    <div className="bg-slate-50 border border-slate-200 rounded-[1.5rem] p-6 relative mb-8">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="md:col-span-2">
                                                ...
                                                <div className="relative">
                                                    <select id="wrap-namaPengganti" onChange={handlePenggantiChange} value={formData.namaPengganti} className={`block w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-[15px] font-semibold shadow-sm transition-all duration-300 focus:border-blue-500 focus:outline-none focus:ring-[4px] focus:ring-blue-500/10 appearance-none relative z-10 cursor-pointer ${formData.namaPengganti ? 'text-slate-800' : 'text-slate-400 font-medium'}`}>
                                            ...
                                            <div className={`transition-all duration-500 overflow-hidden md:col-span-2 ${(formData.namaPengganti && formData.namaPengganti !== 'TIADA PENGGANTI') ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-1">
                                                    ...
                                                    <div className="md:col-span-2">
                                                        <UniversalSelect name="jenisAmbilAlih" value={formData.jenisAmbilAlih} label="Jenis Ambil Alih Tugas" options={["Ambil alih subjek / tugas sepenuhnya", "Ambil alih kawalan kelas / tugas"]} onChange={handleChange} />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-7 mb-8">
                                        ...
                                        <div className="md:col-span-2 flex items-center justify-end gap-3 mb-[-15px]">
                                            <span className="text-[12px] font-bold text-slate-500">{isGantiDateLocked ? '🔒 Tarikh ikut tugasan' : '🔓 Tarikh bebas'}</span>
                                            <button type="button" onClick={() => setIsGantiDateLocked(!isGantiDateLocked)} className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none ${isGantiDateLocked ? 'bg-slate-300' : 'bg-emerald-500'}`}>
                ...
                <div id="jana-button-container" className="mt-12 mb-16 animate-slide-up" style={{animationDelay: '0.5s'}}>
                    <button 
                        onClick={handleGenerateAll} disabled={isGenerating || isLogoLoading}
                    ...
                    >
                        ...
                        <div className="relative z-10 flex items-center gap-3 text-white">
                            ...
                            <span className="text-[14px] sm:text-[16px] font-extrabold tracking-wide uppercase">
                                {isLogoLoading ? 'SEDANG MEMUATKAN ASET BORANG' : isGenerating ? 'MENJANA BORANG... SILA TUNGGU' : (!isPegawaiComplete ? 'KLIK UNTUK ISI MAKLUMAT PEGAWAI' : (activeForm === 'tugas' && !isTugasComplete) ? 'KLIK UNTUK ISI MAKLUMAT TUGASAN' : (activeForm === 'tugas' && !isPenggantiComplete) ? 'KLIK UNTUK ISI MAKLUMAT PENGGANTI' : (activeForm === 'cuti' && !isCutiComplete) ? 'KLIK UNTUK ISI MAKLUMAT CUTI' : (activeForm === 'akujanji' && !isPerananComplete) ? 'KLIK UNTUK ISI PERANAN' : (activeForm === 'akujanji' && !isTandatanganComplete) ? 'KLIK UNTUK ISI TANDATANGAN' : (activeForm === 'tugas' && !isTiketComplete) ? 'KLIK UNTUK ISI MAKLUMAT TIKET' : 'JANA & MUAT TURUN (FAIL PDF)')}
                            </span>
                        </div>
                    </button>
                </div>
