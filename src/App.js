...
const FeedbackButton = () => {
  ...
  // Scroll detection (unchanged, works fine)
  ...
  const generatePDFBase64 = (name, email, message, timestamp, starRating) => {
    ...
    doc.text(`Emel: ${email || "Tiada emel"}`, 15, 54);
  ...
  const handleSubmit = async () => {
    if (rating === 0 || !feedbackMessage.trim()) {
      setSubmitError("Sila berikan rating dan maklum balas anda.");
      return;
    }
    ...
    try {
      ...
      const { pdfBase64, filename } = generatePDFBase64(
        feedbackName, feedbackEmail, feedbackMessage, now, rating
      );
      
      const formData = new FormData();
      formData.append('filename', filename);
      formData.append('base64', pdfBase64);
      formData.append('email', feedbackEmail);      // send email to script
      ...
      const response = await fetch(GOOGLE_DRIVE_FEEDBACK_URL, {
        method: 'POST',
        body: formData,
        // mode: 'cors' is default; ensure Apps Script returns Access-Control-Allow-Origin: *
      });
      ...
      const result = await response.json(); // assuming script returns { success: true }
      ...
      setFeedbackName('');
      setFeedbackEmail('');
      alert("✓ Terima kasih! Maklum balas anda disimpan ke sistem iFMS.");
    } catch (err) {
      console.error("Submit error:", err);
      setSubmitError("Gagal menghantar maklum balas. Sila cuba sebentar lagi.");
    } finally {
      setIsSubmitting(false);
    }
  };

  // Helper to reset error when modal opens
  const openModal = () => {
    setSubmitError('');
    setIsModalOpen(true);
  };
  ...
  return (
    <>
      {/* Floating button with improved desktop visibility */}
      <div className={`fixed z-[9999] transition-all duration-500 ease-in-out ${
        isVisible 
      ...
      {/* Modal - Fixed close icon & added error display */}
      {isModalOpen && (
            ...
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto">
              <div className="text-center space-y-3">
                ...
                <div className="flex justify-center gap-2">
                  {[1, 2, 3, 4, 5].map((s) => (
                    <button key={s} onMouseEnter={() => setHoverRating(s)} onMouseLeave={() => setHoverRating(0)} onClick={() => setRating(s)} className="transition-transform hover:scale-125 active:scale-90">
                      <svg className={`w-10 h-10 ${(hoverRating || rating) >= s ? 'text-amber-400' : 'text-slate-200'}`} fill="currentColor" viewBox="0 0 20 20">
              ...
              <div className="space-y-4">
                <input type="text" placeholder="Nama Lengkap (Pilihan)" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none" value={feedbackName} onChange={(e) => setFeedbackName(e.target.value)} />
                <input type="email" placeholder="Emel (Pilihan)" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-blue-500 transition-all outline-none" value={feedbackEmail} onChange={(e) => setFeedbackEmail(e.target.value)} />
                <textarea placeholder="Ceritakan pengalaman anda atau cadangkan sesuatu..." rows="3" className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3.5 text-sm focus:ring-2 focus:ring-blue-500 transition-all resize-none outline-none" value={feedbackMessage} onChange={(e) => setFeedbackMessage(e.target.value)} />
              </div>
              {submitError && (
              ...
              <button
                ...
                disabled={isSubmitting || rating === 0 || !feedbackMessage.trim()}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-200 transition-all flex items-center justify-center gap-3 disabled:bg-slate-300 disabled:shadow-none"
              >
