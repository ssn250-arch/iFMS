...
  // --- SCROLL DETECTION LOGIC (Diperkemas) ---
...
      
      // Sembunyi jika skrol ke bawah > 50px, tunjuk jika skrol ke atas
...
  // --- HANTAR LOGIK (Tanpa Loading Lama) ---
  const handleSubmit = () => {
...
      
      // Menggunakan 'no-cors' untuk fire-and-forget, tutup modal segera
      fetch(GOOGLE_DRIVE_FEEDBACK_URL, { method: 'POST', mode: 'no-cors', body: formData })
        .then(() => {
          // Logik tambahan jika diperlukan, tetapi tiada maklum balas diperlukan dengan no-cors
        })
        .catch(error => {
          console.error("Ralat semasa menghantar:", error);
        });

      // Tunjuk alert dan tutup modal SEGERA untuk mengelakkan loading lama
...
      alert("Maaf, ralat berlaku semasa menjana laporan maklum balas.");
      setIsSubmitting(false);
    }
  };
  // -------------------------------------------
...
  return (
    <>
      {/* 1. FLOATING BUTTON DI SEBELAH KIRI DENGAN ANIMASI HIDE-ON-SCROLL */}
      <div 
        className={`fixed z-[9999] transition-all duration-500 ease-in-out ${
          isVisible 
            // Posisi Kiri: bottom-6 left-6 (mobile), md:bottom-10 md:left-10 (desktop)
            ? 'bottom-6 left-6 md:bottom-10 md:left-10 translate-y-0 opacity-100' 
            // Sembunyi: turun ke bawah pada mobile. Sentiasa tunjuk pada desktop (md).
            : 'bottom-[-100px] left-6 md:bottom-10 md:left-10 md:translate-y-0 opacity-0 md:opacity-100 pointer-events-none md:pointer-events-auto'
        }`}
      >
        <button
...
      {/* 2. MODAL SYSTEM DENGAN TANDA PANGKAH 'X' DIBAIKI */}
      {isModalOpen && (
...
            {/* Bar penunjuk pada mobile */}
            <div className="w-12 h-1.5 bg-slate-200 rounded-full mx-auto mt-4 mb-2 sm:hidden"></div>
            
            {/* Header modal */}
            <div className="px-8 py-4 flex justify-between items-center border-b border-slate-50 relative">
              <h2 className="text-xl font-bold text-slate-800">Suara Anda</h2>
              
              {/* Tanda Pangkah 'X' dibaiki dan diposisikan dengan betul */}
              <button 
                onClick={() => setIsModalOpen(false)} 
                className="flex items-center justify-center p-2.5 bg-slate-100/70 hover:bg-slate-200 rounded-full text-slate-500 hover:text-slate-700 transition-all shadow-[0_2px_10px_rgba(0,0,0,0.05)]"
              >
...
              </div>
              
              {/* Submit Button dengan teks Menghantar... */}
              <button
