...
const FormTugas = ({
    ...
    return (
        <>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mt-3">
                                {quickTemplates.map((t, idx) => (
                                    <button 
                                        ...
                                        className="flex flex-col items-center justify-center p-3 rounded-2xl bg-white border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-700 transition-all text-slate-600 font-bold text-[13px] gap-2 active:scale-95 shadow-sm text-center"
                                    >
            ...
            <div id="section-pengganti" className={`bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border overflow-hidden transition-all duration-500 ${!isTugasComplete ? 'border-slate-200/50 opacity-60 grayscale-[20%]' : (expanded.pengganti ? 'border-slate-100 ring-[3px] ring-emerald-500/20' : 'border-slate-100 hover:shadow-md')} ${shakeSection === 'pengganti' ? 'animate-shake border-red-400' : ''} mt-5`}>
                                    ...
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            {pegawaiDatabase.filter(p => p.nama !== formData.nama).sort((a,b) => a.nama.localeCompare(b.nama)).map((p, idx) => (
                                                <option key={`semua-${idx}`} value={p.nama}>{p.nama} ({p.bahagian})</option>
                                            ))}
                                        </select>
            ...
            {/* 5. TANDATANGAN DIGITAL (UNTUK TUGAS RASMI) */}
            <div id="section-tandatangan" className={`transition-all duration-700 overflow-hidden ${expanded.tandatangan ? 'mt-5' : 'm-0 max-h-0 opacity-0'}`}>
                <div className={`bg-white/80 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border overflow-hidden transition-all duration-500 ${(expanded.tandatangan ? 'border-slate-100 ring-[3px] ring-purple-500/20' : 'border-slate-100 hover:shadow-md')} ${shakeSection === 'tandatangan' ? 'animate-shake border-red-400' : ''}`}>
                    <div onClick={() => (isTugasComplete && isPenggantiComplete) && toggleSection('tandatangan')} className={`px-6 py-5 flex items-center justify-between transition-colors ${!(isTugasComplete && isPenggantiComplete) ? 'bg-slate-50/50 cursor-not-allowed' : 'bg-white hover:bg-slate-50 cursor-pointer'}`}>
                        <div className="flex items-center gap-4">
                            <div className={`p-2.5 rounded-xl transition-colors ${!(isTugasComplete && isPenggantiComplete) ? 'bg-slate-200 text-slate-500' : (expanded.tandatangan ? 'bg-purple-600 text-white shadow-md shadow-purple-500/30' : (isTandatanganComplete ? 'bg-emerald-50 text-emerald-600' : 'bg-purple-50 text-purple-600'))}`}>
                            ...
                            <div>
                                ...
                                {!expanded.tandatangan && (isTugasComplete && isPenggantiComplete) && <p className="text-[13px] text-slate-500 font-semibold mt-0.5">{isTandatanganComplete ? 'Tandatangan telah disimpan' : 'Sila lukis atau muat naik sign'}</p>}
                            </div>
                        </div>
                        {(isTugasComplete && isPenggantiComplete) && (
                    ...
                    {expanded.tandatangan && (isTugasComplete && isPenggantiComplete) && (
                        <div className="p-6 md:p-8 pt-2 border-t border-slate-100 animate-slide-up">
